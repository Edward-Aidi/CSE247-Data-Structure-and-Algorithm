...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	private void moveItem(int from, int to) {
        Decreaser<T> temp = this.array[from];
        int location;
        location = this.array[from].loc;
        this.array[from].loc = this.array[to].loc;
        this.array[to].loc = location;
	...
	private void heapify(int where) {
		...
		//
		int left = left(where);
        int right = right(where);
        int smallest = -1;
        
        // find the smaller between current its children.
        if (left < this.size() && this.array[left].getValue().compareTo(this.array[where].getValue()) < 0) {
            smallest = left;
        } else {
            smallest = where;
        }

        if (right < this.size() && this.array[right].getValue().compareTo(this.array[smallest].getValue()) < 0) {
            smallest = right;
        }

        if (smallest != where) {
            moveItem(where, smallest);
            heapify(smallest);
            ticker.tick();
        }
    }
