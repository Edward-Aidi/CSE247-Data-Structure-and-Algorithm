...
public class ShortestPaths {
	...
	public void run() {
		...
		// FIXME
		while(pq.size() > 0) {
			Vertex midVertex = pq.peek(1).getVertex();
			for (Edge e : midVertex.edgesFrom()) {
				int distance = weights.get(e);
				VertexAndDist successor = new VertexAndDist(e.to, distance + pq.peek(1).getDistance());
				if(successor.compareTo(map.get(e.to).getValue()) < 0) {
					map.get(e.to).decrease(successor.sameVertexNewDistance(successor.getDistance()));
					toEdge.put(e.to, e);
				}
			}
			pq.extractMin();
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		// FIXME
		//
		Vertex midVertex = endVertex;
		while(midVertex != startVertex) {
			path.addFirst(toEdge.get(midVertex));
			midVertex = toEdge.get(midVertex).from;
		}
		return path;
	}
